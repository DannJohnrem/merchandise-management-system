    ...
    <div class="max-w-lg w-full bg-white shadow-lg rounded-lg p-6 mt-10 dark:bg-zinc-900 dark:text-white">
        ...
        <div class="space-y-3 text-sm">
            ...
            <p>
                ...
                {{ $item->purchase_cost !== null ? '₱ ' . number_format($item->purchase_cost, 2) : '-' }}
            </p>

            <p><strong>Supplier:</strong> {{ $item->supplier ?? '-' }}</p>

            <p><strong>Assigned Employee:</strong> {{ $item->assigned_employee ?? '-' }}</p>

            <p><strong>Department:</strong> {{ $item->department ?? '-' }}</p>

            <p><strong>Asset Class:</strong> {{ $item->asset_class ?? '-' }}</p>
            ...
            <p>
                ...
                {{ $item->condition ? ucfirst($item->condition) : '-' }}
            </p>
            ...
            <p><strong>Purchase Order No:</strong> {{ $item->purchase_order_no ?? '-' }}</p>

            <p>
                <strong>Warranty Expiration:</strong>
                {{ $item->warranty_expiration?->format('Y-m-d') ?? '-' }}
            </p>
